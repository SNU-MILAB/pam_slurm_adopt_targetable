...
static uint32_t _read_ssh_target_job_id(struct passwd *pwd) {
	// workaround for nfs client not showing file right after creating in nfs server
	// accessing directory seems to force update nfs cache
    if (!opendir(pwd->pw_dir)) {
		return 0;
    }

    const char *jobid_file_name = "/.SSH_TARGET_SLURM_JOB_ID";
    int path_str_len = strlen(pwd->pw_dir) + strlen(jobid_file_name) + 1;
    char *jobid_file_path = malloc(path_str_len);
    uint32_t target_job_id = 0;

    strcpy(jobid_file_path, pwd->pw_dir);
    strcat(jobid_file_path, jobid_file_name);

    // Get value
	FILE *fp = fopen(jobid_file_path, "r");
    ...
    sscanf(buffer, "%"SCNu32, &target_job_id); // convert string to uint32_t
	fclose(fp);
	remove(jobid_file_path);
	free(jobid_file_path);
	free(buffer);
	return target_job_id;
}

static int _try_checking_target_ssh_id(pam_handle_t *pamh, struct passwd *pwd, List steps)
{

	debug("Trying checking target ssh id");
	uint32_t target_job_id = _read_ssh_target_job_id(pwd);
	if (target_job_id == 0) {
		debug("Target job not set!");
		return PAM_IGNORE;
	}
	bool target_job_found = false;
	ListIterator itr = NULL;
	step_loc_t *stepd = NULL;
	...
	while ((stepd = list_next(itr))) {
		if ((stepd->step_id.step_id == SLURM_EXTERN_CONT) &&
		    (pwd->pw_uid == _get_job_uid(stepd)) &&
		    (stepd->step_id.job_id == target_job_id)) {
				debug("Target job found!");
...
static int _action_unknown(pam_handle_t *pamh, struct passwd *pwd, List steps)
{
	...
	if (rc == PAM_SUCCESS) {
		...
		if (_adopt_process(pamh, getpid(), stepd) == SLURM_SUCCESS) {
			send_user_msg(pamh, "Found multiple slurm jobs running on the node. Adopting to the latest job.");
			return PAM_SUCCESS;
		}
				...
				__attribute__((unused)), int argc, const char **argv)
{
	...
	rc = _try_checking_target_ssh_id(pamh, &pwd, steps);
	if (rc == PAM_SUCCESS)
		goto cleanup;
